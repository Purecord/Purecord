...
function parseFiniteSettingNumber(value: unknown) {
    let numberValue = NaN;
...
function getSlideshowIntervalMinutes() {
    const interval = settings.store.slideshowInterval ?? DEFAULT_SLIDESHOW_INTERVAL_MINUTES;
    return interval >= MINUTE_MS ? interval / MINUTE_MS : interval;
}

function getSlideshowIntervalMs() {
    const interval = getSlideshowIntervalMinutes();
    return (Number.isFinite(interval) && interval > 0 ? interval : DEFAULT_SLIDESHOW_INTERVAL_MINUTES) * MINUTE_MS;
}

function getSlideshowIntervalLabel() {
    const interval = getSlideshowIntervalMinutes();
    const minutes = Number.isFinite(interval) && interval > 0 ? interval : DEFAULT_SLIDESHOW_INTERVAL_MINUTES;

    return `Every ${formatNumber(minutes)} ${minutes === 1 ? "minute" : "minutes"}`;
}

function normalizeSlideshowIntervalSetting() {
    const interval = settings.store.slideshowInterval;
    if (typeof interval === "number" && Number.isFinite(interval) && interval >= MINUTE_MS) {
        settings.store.slideshowInterval = Number(formatNumber(interval / MINUTE_MS));
    }
}
...
const settings = definePluginSettings({
    ...
    slideshowInterval: {
        ...
        hidden: () => !settings.store.enableSlideshow,
        isValid: value => {
            const interval = parseFiniteSettingNumber(value);
            return interval != null && interval > 0 ? true : "Slideshow interval must be greater than 0.";
        },
        onChange: () => {
    ...
    transitionDuration: {
        ...
        hidden: () => !settings.store.enableSlideshow,
        isValid: value => {
            const duration = parseFiniteSettingNumber(value);
            return duration != null && duration >= 0 ? true : "Transition duration cannot be negative.";
        },
        onChange: notifyUI
    },
    overwriteCSS: {
    ...
    blur: {
        ...
        stickToMarkers: false,
        componentProps: {
            onValueRender: (value: number) => `${Math.round(value)}px`
        },
        onChange: notifyUI
    },
    grayscale: {
...
function BgStyleInjector() {
    ...
    const transition = pluginSettings.enableTransition ? (pluginSettings.transitionDuration ?? DEFAULT_TRANSITION_DURATION_MS) : 0;
    const dimming = (pluginSettings.dimming ?? 0) / 100;
    const blur = pluginSettings.blur ?? 0;
    const filterParts = [
        `grayscale(${pluginSettings.grayscale ?? 0}%)`,
        `contrast(${pluginSettings.contrast ?? 100}%)`,
        `saturate(${pluginSettings.saturate ?? 100}%)`
    ];
    if (blur > 0) {
...
function MediaThumbnail({ media }: { media: MediaItem; }) {
    ...
    return (
        <button
            type="button"
            aria-label={`Select ${media.kind} background`}
...
function ManagerPopout() {
    ...
    const pluginSettings = settings.use(["enableSlideshow", "slideshowInterval"]);
    const slideshowIntervalLabel = getSlideshowIntervalLabel();
                ...
                <div className={cl("input-row")}>
                    <div className={cl("slideshow-control")}>
                        <div className={cl("slideshow-meta")}>
                            ...
                            <span className={cl("slideshow-note")}>{slideshowIntervalLabel}</span>
                        </div>
                        <Switch checked={pluginSettings.enableSlideshow} onChange={toggleSlideshow} />
                    </div>
                    <Tooltip text="Upload Backgrounds">
                        {tooltipProps => (
                            <button {...tooltipProps} type="button" aria-label="Upload backgrounds" className={cl("btn", "btn-upload")} onClick={handleUpload}>
                    ...
                    <Tooltip text="Remove Background">
                        {tooltipProps => (
                            <button {...tooltipProps} type="button" aria-label="Remove background" className={cl("btn", "btn-remove")} onClick={deselectAll}>
                                <SvgIcon d={ICONS.remove} />
                            </button>
                        )}
                    </Tooltip>
                </div>
                {storedMedia.length > 0 && pluginSettings.enableSlideshow && storedMedia.length >= 2 && (
                    <div className={cl("info")}>
                        <span>{slideshowIntervalLabel}</span>
                        <Tooltip text="Next Background">
                            {tooltipProps => (
                                <button {...tooltipProps} type="button" aria-label="Next background" className={cl("btn", "btn-next")} onClick={nextImage}>
...
export default definePlugin({
    ...
    async start() {
        ...
        normalizeSlideshowIntervalSetting();
