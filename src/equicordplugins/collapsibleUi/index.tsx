...
import { PanelId, panelRegistry, setCollapseSettingChangeHandler, settings, setUserAreaDetachSettingChangeHandler, toolbarPanelOrder } from "./settings";
...
function isUserAreaNode(node: unknown) {
    return node instanceof Node && userAreaElement?.contains(node);
}
...
function isChannelListNode(node: unknown) {
    return node instanceof Node && channelListElement?.contains(node);
}
            ...
            addSurfacePropsProvider("sidebar", () => {
                ...
                return {
                    ...
                    onBlurCapture: event => {
                        ...
                        if (isChannelListEvent(event) && !isChannelListNode(event.relatedTarget)) {
                            setChannelListExpandedByInteraction(false);
                            return;
                        }
                        if (containsRelatedTarget(event)) return;
